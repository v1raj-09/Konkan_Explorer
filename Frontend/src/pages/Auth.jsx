...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f0d] via-[#111814] to-[#050806] text-white font-sans flex flex-col justify-between selection:bg-emerald-500 selection:text-black">
      
      {/* Navbar (Fixed आहे त्यामुळे वरून स्पेस सोडली आहे) */}
      <Navbar />

      {/* Main Content Area */}
      <div className="flex-grow flex items-center justify-center px-4 pt-28 pb-16 relative z-10 w-full">
        
        {/* Card Container */}
        <div className="w-full max-w-3xl bg-black/70 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_10px_30px_rgba(16,185,129,0.15)] flex flex-col md:flex-row overflow-hidden">
          
          {/* FORM PANEL */}
          <div className={`w-full md:w-1/2 p-8 sm:p-12 flex items-center justify-center bg-black/40 transition-all duration-500 ${!isLogin ? 'md:order-2' : 'md:order-1'}`}>
            <div className="w-full max-w-sm">
              ...
              <p className="text-center text-xs text-gray-400 mb-6 uppercase tracking-wider">
                {isLogin ? "or use your account" : "or use email for registration"}
              </p>
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                {!isLogin && (
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-black/60 border border-white/10 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-emerald-400 transition"
                  />
                )}
                <input
                  type="email"
                ...
                {isLogin && (
                  <div className="text-center pt-1">
                    <a href="#forgot" className="text-xs text-gray-400 hover:text-emerald-300 transition">
              ...
              <div className="mt-6 text-center">
                <Link to="/" className="text-xs text-gray-400 hover:text-emerald-300 transition">
                  ← Back to Home
                </Link>
              </div>
            </div>
          ...
          {/* SIDE INFO PANEL */}
          <div className={`w-full md:w-1/2 p-8 sm:p-12 flex items-center justify-center text-center bg-gradient-to-br from-[#0D3B3E]/90 via-[#123B3E]/70 to-black/95 text-white transition-all duration-500 border-l border-white/10 ${!isLogin ? 'md:order-1' : 'md:order-2'}`}>
            <div className="max-w-xs">
              <h3 className="text-2xl font-serif font-light mb-3 text-white">
                {isLogin ? "Welcome Back Traveler!" : "Explore Konkan Heritage"}
              </h3>
